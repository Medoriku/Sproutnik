...
/**
 * @brief Get CO2 concentration as unsigned 16-bit ppm.
 *
 * This returns the driver value (uint16_t) after the driver's internal byte-swap
 * so the caller sees the correct unsigned ppm directly.
 */
Error_t PASCO2Ino::getCO2u(uint16_t & CO2PPM)
{
    int32_t ret = XENSIV_PASCO2_OK;
    ...
    /* Read the data using the core API that returns a uint16_t (driver applies htons internally) */
