...
int32_t xensiv_pasco2_plat_i2c_transfer(void * ctx, uint16_t dev_addr, const uint8_t * tx_buffer, size_t tx_len, uint8_t * rx_buffer, size_t rx_len)
{
    ...
    if(NULL != rx_buffer)
    {
        ...
        // NOTE: calling endTransmission() after requestFrom() is non-standard
        // and can cause communication issues on some cores. The requestFrom
        // already handles the transfer, so do not call endTransmission here.
    }
