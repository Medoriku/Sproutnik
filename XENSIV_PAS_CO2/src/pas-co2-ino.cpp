    ...
    if (cback != nullptr)
{
    ...
    // Use PinStatus for MBED (Arduino GIGA), uint8_t for others
    #if defined(ARDUINO_ARCH_MBED)
        PinStatus int_event = RISING;
    #else
    ...
    {
        /* In this case, trigger on both edges */
        int_event = CHANGE;
    }

    /* Enable MCU interrupt */
    #if defined(ARDUINO_ARCH_MBED)
        attachInterrupt(intPin, (voidFuncPtr)cback, (PinStatus)int_event);
    #else
