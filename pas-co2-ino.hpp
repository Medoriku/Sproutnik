...
class PASCO2Ino
{
    ...
    /**
     * @brief Get CO2 concentration as unsigned 16-bit value (ppm)
     *
     * Returns the raw driver result (uint16_t) after the internal byte-swap so
     * the caller receives the unsigned ppm directly and avoids signed interpretation issues.
     */
    Error_t getCO2u         (uint16_t & CO2PPM);
